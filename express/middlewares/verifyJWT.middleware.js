...
module.exports = function (req, res, next) {
  try{  console.log(':: token middleware');
    ...
    if (!token || token[0] !== 'Bearer' || !token[1]) return middlewareError(next)( new MiddlewareError(401, 'token middleware error, no token for request', 'No Token Supplied, Access denied.') )
    jwt.verify(token[1], process.env.ACCESS_TOKEN_SECRET
      , async(err, userTokenValues) => {
        if (err) { return middlewareError(next)( new MiddlewareError(403, 'token middleware error, wrong token', 'Token Error, No Access.') ); }
        ...
        if(!user) return middlewareError(next)( new MiddlewareError(403, 'token middleware error, no found user for this token', 'No user found for this token, No Access.') )
        req.user = user;  //console.log(userTokenValues);
        next();
      }
    );
    } catch(e){ return middlewareError(next)( new MiddlewareError(500, 'token middleware error, token system fail', 'token system fail') ); }
};
