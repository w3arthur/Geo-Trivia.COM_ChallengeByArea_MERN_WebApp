...
userRouter.route('/') //  localhost:3000/api/users
  .post(userValidator, async (req, res, next) => {
    ...
    //Add try/catch
      ...
      if (user !== null) throw error( new ErrorHandler(452, 'user already exist!') );
  ...
  .get(async (req, res, next) => { 
    ...
    errorHandler(req, res, next)( async () => {
      ...
      if (!users) throw error(new ErrorHandler(400, 'no users') );
      return success(req, res)( new Success(200, users) );
    });  //error handler 
  })
  ;
