...
loginRouter.route('/') //  /api/login
  .post( validateUser, async (req, res, next) => {
    ...
    errorHandler(req, res, next)( async () => {
      ...
      if(!await bcrypt.compare(req.body.password, user.password)) throw error(400, 'user or password is not correct!');
      ...
      return success(req, res)(200, { email: user.email, accessToken: accessToken, refreshToken: refreshToken });  //
     });  //error handler
  })
  .patch( (req, res, next) => {
    ...
    if(!refreshToken) throw error(401, 'Refresh Token missing.');
      jwt.verify(refreshToken, refreshTokenSecret, (err, userTokenValue) => {
          ...
          return success(req, res)(200, {email: userTokenValue.email, accessToken: accessToken}); //add type/role and name //fix get email/name from database
      });
    });  //error handler 
  })
  .delete( (req, res, next) => {
    ...
    errorHandler(req, res, next)( async () => {
      const refreshToken = ( req.cookies && req.cookies[cookieName]) || req.headers['authorization'] || req.header['x-auth-token'] || req.body['token'] || req.query['token'];
      if(!refreshToken) return success(req, res)( 204 );
...
function validateUser(req, res, next) {
  console.log(':: login validator middleware');
    ...
    if (error && error.details) return middlewareError(next)(400, 'validation user login info error', error.details[0].message.toString());
    next();
}
